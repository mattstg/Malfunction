...
public class TutorialFlow : Flow
{
    ...
    public void SetTutorialText(string jsonName)
    {
		...
		LoLSDK.LOLSDK.Instance.SpeakText("TTS_" + jsonName);
    }
    ...
    public void InputSubmitPressed()
    {
        if (isFirstFieldSubmit)
        {
			...
			else
			{
				...
				LoLSDK.LOLSDK.Instance.SpeakText("TTS_Tutorial_7_Wrong");
			}
        }       
        else
        {
			...
			else
			{
				...
				LoLSDK.LOLSDK.Instance.SpeakText("TTS_Tutorial_14_Wrong");
			}
        }     
    }
